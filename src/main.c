...
static void handle_minute_tick(struct tm* tick_time, TimeUnits units_changed) {
  ...
  strftime(time_text, sizeof(time_text), "%R:%M %p", tick_time);
...
static void do_init(void) {
  ...
  GFont minecraft_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_GAMEBOY_26));
  GFont minecraft_font_small = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_GAMEBOY_12));

  // Init the text layer used to show the time
  time_layer = text_layer_create(GRect(2, 53, 144, 34/* height */));
  ...
  date_layer = text_layer_create(GRect(2, 91, frame.size.w /* width */, 34/* height */));
  ...
  battery_layer = text_layer_create(GRect(30, 115, /* width */ frame.size.w, 34 /* height */));
  ...
  charge_layer = text_layer_create(GRect(30, 115, /* width */ frame.size.w, 34 /* height */));
